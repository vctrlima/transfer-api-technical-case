...
public class ValidateLimitStep implements SagaStep<SagaContext> {
    ...
    public void execute(SagaContext context) {
        ...
        BigDecimal newAccumulated = dailyLimitRepository.incrementAndGet(
                context.getOriginAccountId(),
                context.getAmount()
        );

        if (newAccumulated.compareTo(dailyLimit) > 0) {
            dailyLimitRepository.decrement(context.getOriginAccountId(), context.getAmount());
    ...
    public void compensate(SagaContext context) {
        log.info("[SAGA][Step3:ValidateLimit][Compensate] Revertendo incremento Redis para transferId={}", context.getTransferId());
        dailyLimitRepository.decrement(context.getOriginAccountId(), context.getAmount());
    }
}
